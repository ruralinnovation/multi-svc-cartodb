...
# If PGUSER is not set, set to `postgres`
...
if test x"$PREPARE_PGSQL" = xyes; then
  ...
  echo "\nPreparing postgres on host '$PGHOST'..."
  echo "PostgreSQL server version: `psql -A -t -c 'select version()' -h $PGHOST -U $PGUSER`"
  echo "PAUSE; RESUME;" | psql -h $PGHOST -U $PGUSER -p 6432 pgbouncer # make sure there are no connections pgbouncer -> test_db
